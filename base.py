...
async def genCategories(user_id):
    ...
    if categories != None:
        ...
        key.add(types.InlineKeyboardButton(text = 'Без категории', callback_data='none_categor'))
        key.add(types.InlineKeyboardButton(text='Новая категория', callback_data='new_categor'))
        await db.commit()
        await db.close()
        return key
    else:
...
async def new_file(user_id, title, file_id):
    db = await conn()
    sql = await db.cursor()
    await sql.execute("INSERT INTO files VALUES (NULL, ?, ?, ?, ?)", (user_id, file_id, title, 0))
    await db.commit()
    await db.close()
